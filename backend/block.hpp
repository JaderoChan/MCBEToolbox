#pragma once

#include <map>
#include <string>
#include <string_view>
#include <utility>

#include "color.hpp"
#include "version.hpp"

/** 方块属性，可使用逻辑或运算符进行组合。 */
enum BlockAttribute : int
{
    BLOCK_ATTRI_IS_INCOMPLETE     = 0x00000001, // 非完整方块
    BLOCK_ATTRI_IS_TRANSPARENT    = 0x00000002, // 透明/半透明方块
    BLOCK_ATTRI_IS_LUMINOUS       = 0x00000004, // 发光体
    BLOCK_ATTRI_IS_UNSTABLE       = 0x00000008, // 不稳定（会随时间或环境的影响，自发发生变化）
    BLOCK_ATTRI_IS_CREATIVE       = 0x00000010, // 仅创造模式可获得
    BLOCK_ATTRI_HAS_GRAVITY       = 0x00000020, // 具有重力
    BLOCK_ATTRI_HAS_PATTERN       = 0x00000040, // 具有明显花纹/纹理
    BLOCK_ATTRI_FLAMMABLE         = 0x00000080, // 可燃烧
    BLOCK_ATTRI_ENDERMAN_PICKABLE = 0x00000100  // 可被末影人拿起
};
using BlockAttributes = int;
/** 可用于筛选 #BlockDataMap，不允许任何方块属性 */
constexpr BlockAttributes BLOCK_ATTRI_NONE = 0x00000000;
/** 可用于筛选 #BlockDataMap，允许任意方块属性 */
constexpr BlockAttributes BLOCK_ATTRI_ALL  = 0xFFFFFFFF;

/** 描述方块各个面的材质文件路径与颜色。 */
struct BlockSurface
{
    BlockSurface() = default;

    // {材质文件路径 : 方块颜色}
    std::pair<std::string, Rgb> up;
    std::pair<std::string, Rgb> down;
    std::pair<std::string, Rgb> side;
};

/** 方块数据 */
struct BlockData
{
    BlockData() : attributes(0) {}

    std::string     id;         // 方块 ID
    BlockAttributes attributes; // 方块属性
    BlockSurface    surface;    // 方块面数据
};

/** 方块条目 */
struct BlockEntry
{
    BlockEntry() = default;

    std::string name;             // 方块英文名
    Version     minVersion;       // 最低可用游戏版本
    BlockData   defaultBlockData; // 默认方块数据
    std::map<std::string, std::string> nameTranslations; // {语言代码 : 方块译名}（可以为空）
    std::map<Version, BlockData>       variants;         // 不同版本对应的方块数据
};

// {方块ID : 方块条目}
using BlockEntryMap = std::map<const std::string*, const BlockEntry*>;
// {方块ID : 方块数据}
using BlockDataMap  = std::map<const std::string*, const BlockData*>;

/**
 * 从 Json 字符串中解析 #BlockEntryMap。
 *
 * @note #BlockEntryMap 中的数据需要通过 #releaseBlockEntryMap() 进行释放。
 * 释放之后，通过此 #BlockEntryMap 所获得的 #BlockDataMap 等数据也不再有效。
 *
 * @throw std::runtime_error
 * @throw std::invalid_argument
 */
BlockEntryMap parseBlockEntryMapFromJson(std::string_view json);

/**
 * 释放 #BlockEntryMap 中的数据。
 */
void releaseBlockEntryMap(BlockEntryMap& blockEntryMap);

/**
 * 从 #BlockEntryMap 中解析默认方块数据组成的 #BlockDataMap。
 */
BlockDataMap resolveBlockEntryMap(const BlockEntryMap& blockEntryMap);

/**
 * 从 #BlockEntryMap 中解析符合目标版本的 #BlockDataMap。
 */
BlockDataMap resolveBlockEntryMap(const BlockEntryMap& blockEntryMap, Version targetVersion);

/**
 * 从 #BlockDataMap 中筛选出符合方块属性规则的子集。
 */
BlockDataMap filterBlockAttributes(const BlockDataMap& blockDataMap, BlockAttributes attributes);
